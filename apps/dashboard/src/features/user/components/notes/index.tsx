import { getUserNotesAction } from "@/features/user/actions/notes.actions";
...
import { CalendarArrowDown, NotebookPen } from "lucide-react";
...
export async function Notes() {
  ...
  const groupedNotesByDate: Record<string, Note[]> | undefined = notes?.reduce(
    (acc, note) => {
  ...
  return (
    <Card className="w-full  min-h-[300px] max-h-[350px] md:max-h-fit flex flex-col  p-0  overflow-hidden">
      ...
      <CardContent className="p-0  flex-grow overflow-scroll scrollbar-hide">
        {!groupedNotesByDate || notes?.length === 0 ? (
          <NotesEmptyState />
        ) : (
          Object.entries(groupedNotesByDate).map(([date, notes]) => (
            <div key={date}>
